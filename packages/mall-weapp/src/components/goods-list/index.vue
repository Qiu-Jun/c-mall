    ...
    >
        ...
        >
            ...
            >
                <view
                    :class="{
                        'padding-top': scroll && group,
                        'padding-bottom': !scroll && group
                    }"
                    v-for="(item, index) in datas"
                    :key="index"
                    :id="'goods-' + index"
                    ...
                    <view :class="{ 'shadow-normal': scroll }">
                        <view v-if="group" class="cu-bar bg-white solid-bottom">
                            ...
                            >
                                <text
                                    ...
                                    :class="
                                        item.checked
                                            ? 'cuIcon-roundcheckfill text-red'
                                            : 'cuIcon-round text-grey'
                                    "
                        ...
                        <view v-else class="cu-list menu-avatar">
                            <view
                                ...
                                :class="{
                                    'has-icon': right === 'icon',
                                    'has-number': right === 'number',
                                    'has-select': selectable,
                                    'move-cur': touchKey === key
                                }"
                            ...
                            >
                                ...
                                <view class="content goods-item-content">
                                    <view class="text-cut">
                                        {{ goods.name }}
                                        <text class="margin-left-xs text-xs"
                                            >({{ goods.unitName }})</text
                                        >
                                    </view>
                                ...
                                <view class="goods-item-right">
                                    <text v-if="goods.isDown" class="text-red"
                                        >此商品已下架</text
                                    >
                                    <view
                                ...
                                <view class="move" @click="remove(key, index)">
        ...
        >
            ...
            <view class="goods-footer-right">
                <button class="cu-btn bg-red round shadow-blur" @click="settle">
                    去结算
                </button>
            </view>
        </view>
...
<script>
import uniNumberBox from '@/components/uni-number-box/index.vue'
import { isObject, isArray } from '@/utils/types'
import { getStrRealLen } from '@/utils/utils'
...
export default {
    ...
    methods: {
        ...
        itemTouchMove(e) {
            if (this.removeable) {
                this.touchDirection =
                    e.touches[0].pageX - this.touchStartX > 0 ? 'right' : 'left'
            }
        },
        // 计算滚动
        itemTouchEnd(key) {
        ...
        judgeCheckedAll() {
            ...
            Object.entries(datas).forEach(([index, value = {}]) => {
                const { lines = {} } = value
                const checked = !Object.values(lines).some(
                    (goods) => !goods.checked
                )
            ...
            Object.keys(datas).forEach((key) => {
                ...
                Object.keys(lines).forEach((goodsKey) => {
                    ...
                    if (checked) {
                        ...
                        totalAmount = round(
                            add(totalAmount, multiply(salePrice, quantity)),
                            2
                        )
