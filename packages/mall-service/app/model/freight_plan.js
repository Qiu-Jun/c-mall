...
module.exports = (app) => {
  ...
  FreightPlan.query = async ({
    ...
    const { count, rows: list } = await FreightPlan.findAndCountAll(condition)
    return { page, count, list }
  }

  /**
   * @description 根据uuid获取运费方案
  ...
  FreightPlan.setDefault = async ({ uuid, orgUuid }) => {
    ...
    await FreightPlan.update(
      { sysDefault: 0 },
      { where: { sysDefault: 1 }, transaction }
    )
    await FreightPlan.update(
      { uuid, sysDefault: 1 },
      { where: { uuid, orgUuid }, transaction }
    )
    return uuid
  }
  return FreightPlan
