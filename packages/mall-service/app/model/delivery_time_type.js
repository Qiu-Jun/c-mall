...
module.exports = (app) => {
    ...
    const DeliveryTimeType = model.define(
        'deliverytimetype',
        deliveryTimeTypeSchema
    )
    ...
    DeliveryTimeType.remove = async ({ uuid, orgUuid }) => {
        const result = await DeliveryTimeType.destroy({
            where: { uuid, orgUuid }
        })
        checkDelete(result)
        return uuid
    ...
    DeliveryTimeType.query = async ({
        ...
        const { count, rows: list } = await DeliveryTimeType.findAndCountAll(
            condition
        )
