...
module.exports = {
  _,
  // 日期格式化
  formatToDay(date = Date.now()) {
    return dayjs(date).format('YYYY-MM-DD');
  },
  formatToDayNoYear(date = Date.now()) {
    return dayjs(date).format('MM-DD');
  },
  formatToDayStart(date = Date.now()) {
    return dayjs(date).format('YYYY-MM-DD 00:00:00');
  },
  formatToDayEnd(date = Date.now()) {
    return dayjs(date).format('YYYY-MM-DD 23:59:59');
  },
  formatToDayTime(date = Date.now()) {
    return dayjs(date).format('YYYY-MM-DD HH:mm:ss');
  },

  // 获取当前时间相差 count 天的时间
  getDifferDate(count = 0, date = Date.now()) {
    const time = date;
